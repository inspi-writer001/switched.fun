...
export function GiftTipOverlay({
  notification,
  onComplete,
}: GiftTipOverlayProps) {
  ...
  const gift = allGifts.find(
    (g) => g.id === notification.giftType || g.name === notification.giftName
  );
  ...
  useEffect(() => {
    try {
      ...
      if (!notification || !notification.id || !notification.giftType) {
        console.error(
          "Invalid gift notification in GiftTipOverlay:",
          notification
        );
        setIsVisible(false);
        return;
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center pointer-events-none">
      ...
      >
        ...
        <div
          className={`absolute inset-0 ${glowClasses} rounded-3xl animate-pulse`}
        />
        ...
        <div className="relative z-10 text-center space-y-4">
          ...
          <div className="space-y-1">
            <h3
              className={`text-2xl font-bold ${isMegaGift ? "text-yellow-400" : isPremiumGift ? "text-purple-400" : "text-blue-400"}`}
            >
              {notification.giftName || gift?.name}
            </h3>
            <p className="text-lg text-white/90">Gift received!</p>
          </div>

          {/* Tipper info */}
          <div className="flex items-center justify-center gap-4 bg-black/20 rounded-2xl p-4">
            <Avatar
              className={`w-12 h-12 border-2 ${isMegaGift ? "border-yellow-400/50" : isPremiumGift ? "border-purple-400/50" : "border-blue-400/50"}`}
            >
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-gray-300">sent you a gift!</p>
            </div>
          </div>
          ...
          <div className="bg-black/20 rounded-2xl p-4">
            <div
              className={`text-3xl font-bold ${isMegaGift ? "text-yellow-400" : isPremiumGift ? "text-purple-400" : "text-green-400"} mb-1`}
            >
              ${notification.amount}
            </div>
...
export function GiftTipOverlayManager({
  notifications,
  onNotificationComplete,
}: GiftTipOverlayManagerProps) {
  const [currentOverlay, setCurrentOverlay] = useState<TipNotification | null>(
    null
  );
